...
#include	"CommonServices.h"
	...
	//
	// Found at http://www.dre.vanderbilt.edu/Doxygen/Current/html/ace/Sock__Connect_8cpp-source.html:
	//
...
#if( TARGET_OS_WINDOWS_CE )
	#define ComputerNamePhysicalDnsHostname			0
	BOOL	WINAPI		GetComputerNameExA( int NameType, LPSTR lpBuffer, LPDWORD nSize );
	LONG	APIENTRY	RegCreateKey( HKEY hKey, LPCTSTR lpSubKey, PHKEY phkResult );
	LONG	APIENTRY	RegCreateKeyA( HKEY hKey, LPCSTR lpSubKey, PHKEY phkResult );
	LONG	APIENTRY	RegNotifyChangeKeyValue( HKEY hKey, BOOL bWatchSubtree, DWORD dwNotifyFilter, HANDLE hEvent, BOOL fAsynchronous );
#endif
...
mDNSexport mDNSs32	mDNSPlatformUTC( void )
{
	...
	//
	// The time() function isn't available under Windows CE, so we have to roll our own.  The instructions for how to
	// do this come from the documentation for RtlTimeToSecondsSince1970() in the MSDN library.
	//

	SYSTEMTIME	s_now, s_epoch;
	FILETIME	f_now, f_epoch;
	UINT64		u_now, u_epoch;
	mDNSs32		utc = -1;

	// Convert current time to UINT64

	GetSystemTime( &s_now );
	require( SystemTimeToFileTime( &s_now, &f_now ), exit );
	memcpy( &u_now, &f_now, sizeof( f_now ) );

	// Convert epoch (0 hours, 0 minutes, 0 seconds, January 1, 1970) to UINT64
	...
	require( SystemTimeToFileTime( &s_epoch, &f_epoch ), exit );
	memcpy( &u_epoch, &f_epoch, sizeof( f_epoch ) );

	// Compute elapsed time since epoch in seconds
	require( u_epoch >= u_now, exit );
	utc = ( mDNSs32 )( ( u_epoch - u_now ) / ( UINT64 )10000000 );
	...
	if( !gIPHelperLibraryInstance )
	{
		...
		if( gIPHelperLibraryInstance )
		{
			gGetAdaptersAddressesFunctionPtr = 
				(GetAdaptersAddressesFunctionPtr) GetProcAddress( gIPHelperLibraryInstance, TEXT( "GetAdaptersAddresses" ) );
...
mDNSlocal int	getifaddrs_ce( struct ifaddrs **outAddrs )
{
	...
	//
	// Changed by cstawarz, 2006-12-11:
	//   I've never seen an example of this bug, but I have seen instances where there actually are no addresses
	//   in the list (namely, when the device is connected to a PC via a USB cable), in which case setting n to
	//   1 leads directly to a memory error and application crash.  Without more information, this code seems too
	//   risky to leave in place.
...
//===========================================================================================================================
//	Compatibility functions for Windows CE
//===========================================================================================================================

#if( TARGET_OS_WINDOWS_CE )

BOOL WINAPI
GetComputerNameExA( int NameType, LPSTR lpBuffer, LPDWORD nSize )
{
    int status;

	status = gethostname( lpBuffer, *nSize );
	...
	else
	{
		...
		*nSize = 2 * (*nSize);  // We're supposed to return the needed buffer size, so just guess
	}
...
{
    return RegCreateKeyEx( hKey, lpSubKey, 0, NULL, 0, KEY_ALL_ACCESS, NULL, phkResult, NULL );
}
...
{
    LSA_UNICODE_STRING	uBuf;
	LONG				rv;

	if ( MakeLsaStringFromUTF8String( &uBuf, lpSubKey ) )
	    rv = GetLastError();
	else
	    rv = RegCreateKeyExW( hKey, uBuf.Buffer, 0, NULL, 0, KEY_ALL_ACCESS, NULL, phkResult, NULL );
...
LONG APIENTRY
RegEnumKeyExA( HKEY hKey, DWORD dwIndex, LPSTR lpName, LPDWORD lpcbName, LPDWORD lpReserved, LPSTR lpClass, LPDWORD lpcbClass, PFILETIME lpftLastWriteTime )
{
    LSA_UNICODE_STRING	nameBuf;
    LSA_UNICODE_STRING	classBuf;
	LONG				rv;

	classBuf.Buffer = NULL;

	if ( MakeLsaStringFromUTF8String( &nameBuf, lpName ) ||
		 ( ( NULL != lpClass ) && MakeLsaStringFromUTF8String( &classBuf, lpClass ) ) )
	    rv = GetLastError();
	else
	    rv = RegEnumKeyExW( hKey, dwIndex, nameBuf.Buffer, lpcbName, lpReserved, classBuf.Buffer,
							lpcbClass, lpftLastWriteTime );

	if ( nameBuf.Buffer != NULL )
	{
	    free( nameBuf.Buffer );
	}
	if ( classBuf.Buffer != NULL )
	{
	    free( classBuf.Buffer );
	}
...
{
    // No equivalent function is available under CE, so just mimic success
...
RegOpenKeyExA( HKEY hKey, LPCSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult )
{
	...
	if ( MakeLsaStringFromUTF8String( &uBuf, lpSubKey ) )
	    ...
	    rv = RegOpenKeyExW( hKey, uBuf.Buffer, ulOptions, samDesired, phkResult );
...
RegQueryValueExA( HKEY hKey, LPCSTR lpValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData )
{
    ...
    // FIXME: do we need to worry about calling code assuming lpData will be an LPSTR?

	if ( MakeLsaStringFromUTF8String( &uBuf, lpValueName ) )
	    rv = GetLastError();
	else
	    rv = RegQueryValueExW( hKey, uBuf.Buffer, lpReserved, lpType, lpData, lpcbData );
...
INT WSAAPI
WSAStringToAddressA( LPSTR AddressString, INT AddressFamily, LPWSAPROTOCOL_INFOA lpProtocolInfo, LPSOCKADDR lpAddress, LPINT lpAddressLength )
{
    LSA_UNICODE_STRING	uBuf;
	INT					rv;

	// Currently, all calls to this function pass in NULL for lpProtocolInfo, so let's insist on this
	// and avoid having to convert the parameter to a LPWSAPROTOCOL_INFOW
	check( NULL == lpProtocolInfo );

	if ( !MakeLsaStringFromUTF8String( &uBuf, AddressString ) )
	{
	    rv = WSAStringToAddressW( uBuf.Buffer, AddressFamily, NULL, lpAddress, lpAddressLength );
	}
	else
	{
	    rv = SOCKET_ERROR;
